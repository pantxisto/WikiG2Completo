        ...
        try {
            ...
            if ($request = CorreosCommon::getRequestData($id_cart, $id_carrier)) {
                if (isset($request->offices)) {
                    ...
                    if (isset($request->id_address_delivery)) {
                        if ($request->id_address_delivery == $cart->id_address_delivery && $request->reference == $postcode) {
                            return $target == 'front' ? Tools::jsonEncode($request) : Tools::jsonEncode($offices);
                        }
                    } elseif ($request->reference == $postcode) {
                        return $target == 'front' ? Tools::jsonEncode($request) : Tools::jsonEncode($offices);
                    }
                }
            }

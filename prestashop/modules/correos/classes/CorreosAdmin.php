...
class CorreosAdmin
{
    ...
    public static function getOrderData($order, $cart, $carrier_code, $delivery_address, $request_data)
    {
        ...
        if (in_array($carrier_code, $correos->carriers_codes_office) ||
            ...
            $shipping_val['mobile'] = isset($delivery_address->phone_mobile) ? $delivery_address->phone_mobile : "";
        }
        if (in_array($carrier_code, $correos->carriers_codes_office)) {
            $office = false;
        ...
        } else if (in_array($carrier_code, $correos->carriers_codes_homepaq) ||
            in_array($carrier_code, $correos->carriers_codes_citypaq)) {
            $correos_paq = false;
        ...
        } else if (in_array($carrier_code, $correos->carriers_codes_international)) {
    ...
    public static function prepareDataFromPost($order, $cart, $carrier_code, $correos_config, $delivery_address, $request_data = false)
    {
        ...
        'customer_firstname'    => trim($recipient['firstname']),
        'customer_lastname1'    => trim($recipient['lastname']),
        'customer_lastname2'    => '',
        'customer_company'      => trim($recipient['company']),
        ...
        } else if (in_array($carrier_code, $correos->carriers_codes_hourselect)) {
            if (isset($request_data->id_schedule)) {
                $shipping_val['id_schedule'] = $request_data->id_schedule;
            }
        } else if (in_array($carrier_code, $correos->carriers_codes_international)) {
            if (isset($request_data->mobile)) {
                if (!is_object($request_data->mobile) && !empty($request_data->mobile)) {
                    $shipping_val['phone'] = $request_data->mobile;
                } else if (is_object($request_data->mobile) && isset($request_data->mobile->number)) {
                    $shipping_val['phone'] = $request_data->mobile->number;
                }
            }
            //Mobile phone only for Spain. Otherwise the Correos server will respond will error
            $shipping_val['mobile']      = "";
   ...
   /*Prepare data to preregister*/
    public static function prepareData($order, $cart, $carrier_code, $correos_config, $delivery_address)
    {
        ...
        'customer_firstname'    => $delivery_address->firstname,
        'customer_lastname1'     => trim($delivery_address->lastname),
        'customer_lastname2'    => '',
        'customer_company'      => trim($delivery_address->company),
        'delivery_address'      => $delivery_address->address1,
        'delivery_address2'      => $delivery_address->address2,
        ...
        if ($shipping_val['country_iso'] == 'PT') {
            $shipping_val['delivery_zip'] = $delivery_address->postcode;
            $shipping_val['delivery_postcode'] = '';
        }
        ...
        if (in_array($carrier_code, $correos->carriers_codes_office)) {
            ...
            if (isset($data->mobile)) {
                if (!is_object($data->mobile) && !empty($data->mobile)) {
                    $shipping_val['phone'] = $data->mobile;
                } elseif (is_object($data->mobile) && isset($data->mobile->number)) {
                    $shipping_val['phone'] = $data->mobile->number;
                }
            }
    ...
    public static function requestRMALabel()
    {
        ...
        'customer_company'      => trim(Tools::getValue('recipient_empresa')),

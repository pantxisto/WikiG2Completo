...
class Correos extends Module
{
    ...
    public function getContent()
    {
        if (Configuration::get('PS_DISABLE_OVERRIDES') == 1) {
            $this->html .= $this->displayError($this->l('This module works with the overrides. '.
            'Turn off option "Disable all overrides" on Advanced Parameters -> Performance'));
        }
        if (Module::isInstalled('onepagecheckoutps') && version_compare(_PS_VERSION_, '1.7', '<')) {
            ...
            if (version_compare($onepagecheckoutps->version, '2.2.5', '<') && $onepagecheckoutps->active == 1) {
                $this->html .= $this->displayError($this->l('To ensure the correct functioning of your Checkout Page, '.
                'please update the "One Page Checkout PrestaShop" module from PresTeamShop.com at least to the version 2.2.5'));
            }
        }
        $this->postProcess();
    ...
    protected function postProcess()
    {
        ...
        if (Tools::isSubmit('form-request_collection')) {
          ...
          } elseif (count($orders) > 5 && Tools::getValue('collection_req_label_print') == 'S') {
            $this->html .= $this->displayError(
              $this->l('If you request label print, the maximum is 5 labels')
               ...
               $context->smarty->assign(array(
                ...
                if (!empty($dataXml->soapenv_Body->ns3_SolicitudRegistroRecogidaResult->ns3_RespuestaSolicitudRegistroRecogida->CodigoError)) {
                   $this->html .= $this->displayError(
                        $dataXml->soapenv_Body->ns3_SolicitudRegistroRecogidaResult->ns3_RespuestaSolicitudRegistroRecogida->DescripcionError
                    );
                } else {
    ...
    public function hookHeader($params)
    {
        ...
        if (in_array($file, array('order-opc', 'order', 'orderopc', 'history', 'supercheckout', 'amzpayments'))) {
            ...
            if (version_compare(_PS_VERSION_, '1.7', '>')) {
                $this->context->controller->addJS($this->_path.'views/js/front/correos_v426_ps17.js');
            } else {
                $this->context->controller->addJS($this->_path.'views/js/front/correos_v426.js');
            }
            ...
            $correos_message = array(
                ...
                'officeMobileError' => $this->l('If you want to give your mobile phone number, check its format. '.
                    'Need to be correct to inform you when your package is ready to be picked up'),
                'officeValidContactError' => $this->l('To be filled mobile phone number or e-mail'),
                'officeEmailError' => $this->l('If you want to give your E-mail, check its format. '.
                    'Need to be correct to inform you when your package is ready to be picked up'),
    ...
    public function hookdisplayCarrierExtraContent($params)
    {
        ...
        if (in_array($correos_carrier['code'], $this->carriers_codes_office)) {
            ...
            } elseif (!isset($request_data->id_address_delivery) && isset($request_data->reference)) {
                $postcode = $request_data->reference;
            }
            $params_tpl = array(
                'carrier_type' => 'office',
                ...
                'cr_client_mobile' => isset($request_data->mobile->number) && !empty($request_data->mobile->number) ?
                    $request_data->mobile->number : $address->phone_mobile,
                'cr_client_email' => isset($request_data->email) && !empty($request_data->email) ?
                    $request_data->email : $customer->email,
